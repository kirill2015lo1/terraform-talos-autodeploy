...
output "node_info" {
  ...
  value = {
    ...
    ip_address   = proxmox_virtual_environment_vm.node.agent[0].enabled && length(proxmox_virtual_environment_vm.node.ipv4_addresses[7]) > 0 ? proxmox_virtual_environment_vm.node.ipv4_addresses[7][0] : null
